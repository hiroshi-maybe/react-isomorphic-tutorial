...
function createEngine(engineOptions) {
  ...
  function renderFile(filename, options, cb) {
    try {
      ...
      // @CHANGED:
      // Handle layout option on renderer
      var wrapper,
          layout = options.layout;
      if (layout) {
	wrapper = React.createFactory(require(layout));
      }
      ...
      if (!layout) {
	// @CHANGED:
	// Use renderToString() instead of renderToStaticMarkup() to render with 'react-checksum'
	markup += React.renderToString(component(options));
      } else {
	// @CHANGED:
	// If layout is specified, render partial content to provide entry-point to hook on client
	// react-checksum is given only on the root node rendered by renderToString()
        wrapper = React.renderToString(wrapper({
	  initialData: options.initialData,
	  content: React.renderToString(component(options))
	}));
	markup += wrapper;
      }
